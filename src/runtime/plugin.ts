...
export default defineNuxtPlugin(async (nuxtApp) => {
  ...
  class Auth implements AuthInstance {
    ...
    private getHandler(strategyName: string, key: string): string | null {
      return (
        this.options.strategies?.[strategyName]?.handler?.find(
          (value: any) => value[key],
        )?.[key] ?? null
      );
    }
    ...
    async initialize(): Promise<void> {
      try {
        ...
        if (import.meta.server) {
          ...
          if (!event) {
            console.warn(
              'No request event available. Skipping initialization.',
            );
            return;
          }
          ...
          token = strategy
            ? cookies[this._prefix + `_token.` + strategy]
            : null;
        } else {
          strategy = sessionStorage.getItem(this._prefix + `strategy`);
          token = strategy
            ? sessionStorage.getItem(this._prefix + `_token.` + strategy)
            : null;
        }
    ...
    async loginWith(strategyName: string, value: any): Promise<any> {
      try {
        ...
        sessionStorage.setItem(
          this._prefix + `_token.` + strategyName,
          response.token,
        );
        sessionStorage.setItem(this._prefix + `strategy`, strategyName);
        sessionStorage.setItem(
          this._prefix + `_token_expiration.` + strategyName,
          response.expires,
        );
    ...
    async _2fa(
      ...
      try {
        ...
        const response = await $fetch<{ token?: string; expires?: string }>(
          twoFaUrl,
          {
            method: 'POST',
        ...
        if (import.meta.client) {
          sessionStorage.setItem(
            this._prefix + '_2fa.' + strategyName,
            response.token,
          );
          sessionStorage.setItem(
    ...
    async csrfToken(event?: any): Promise<boolean> {
      try {
        ...
        const data = await $fetch<{ csrf_token?: string }>(csrfEndpoint, {
          baseURL,
        });
